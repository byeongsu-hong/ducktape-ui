...
| C06 | P2 · Done | [PR #1064](https://github.com/byeongsu-hong/ducktape-ui/pull/1064), [Content workspace guide](../crates/ui-lang-components/docs/content-workspaces.md), [native evidence](../examples/showcase/screenshots/content-workspaces/README.md) | AI chat preserves historical reading and explicit Latest following through terminal settlement; long links wrap and code scrolls horizontally. Bounded media/editor widths retain actual drafts across resize; raster/SVG cover clips and raster radii are enforced. Embedded native image literals retain decode-cache identity. |
| C07 | P2 · Done | [PR #1064](https://github.com/byeongsu-hong/ducktape-ui/pull/1064), [Native data workspace](../examples/showcase/tests/cases/ui/content_workspace.ice), [guide and limits](../crates/ui-lang-components/docs/content-workspaces.md) | Default data frames compose empty, selected and editing states; native grid/tree drafts survive background appends and commit, historical logs retain selection until explicit tail resumption, and caller-selected width survives resize. Existing 100,000-row fixed-height virtualization and release frame/allocation contracts remain explicit. |
...
| ID | Priority/state | Inspect first | Acceptance scenario |
| --- | --- | --- | --- |
| G01 | P0 · Done | [Authoring guide](ui-authoring.md), [observed agent runs](evidence/agent-authoring/results.md), [PR #1061](https://github.com/byeongsu-hong/ducktape-ui/pull/1061) | Both recorded agents imported the default library and ran the representative Settings screen without catalog-only adapters or an invented import mechanism. Starter and library entry points link the supported source import and screen composition. |
| G02 | P1 · Done | [Canonical compositions](ui-authoring.md), [PR #1061](https://github.com/byeongsu-hong/ducktape-ui/pull/1061) | The guide links compiling Settings, list/detail, dialog and collection compositions, their scroll/state owners and supported customization. Settings keeps its heading and Save outside the short-window form scroller. |
| G03 | P0 · Done | [Coverage ledger](../COVERAGE.md), [native inspection coordinates](testing.md), [PR #1063](https://github.com/byeongsu-hong/ducktape-ui/pull/1063), [Settings evidence](evidence/agent-authoring/results.md) | Relevant width/content/state variants combine geometry, real input and inspected captures. Individual Settings mutations reject lost edits, changed radius and phantom explanation spacing. Nested-scroll paint inspection uses visible screen bounds and rejects hidden-neighbor association in direct and captured output. |
| G04 | P1 · Done | [Protocol](evidence/agent-authoring/protocol.md), [before/after observations](evidence/agent-authoring/results.md), [PR #1061](https://github.com/byeongsu-hong/ducktape-ui/pull/1061) | The same bounded Settings request was run in both conditions; structural mistakes, interventions and time to independent verification are recorded. Both met the UI criteria without repair instructions. Prior context and cache differences prevent a causal speed claim; no authoring improvement is inferred from compile success or line count. |
