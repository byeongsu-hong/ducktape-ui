...
| --- | --- | --- |
| Settings or form | [Settings](../examples/settings/src/ui/app.ice); `cargo test -p settings-example` | `Form` owns scrolling and a 640px width cap. This compact screen sets its padding to 16px; the component default is 24px. State owns values and validation. `TextField` accepts radius and padding without replacing its input. |
