...
Reuse the existing `.ice` `editor-binding` extern kind, arguments and route; add no alternate syntax or separate claims function. For Target::Tree (both native execution and Wasm), the Rust function is `fn(authored_args...) -> ui_lang_guest::EditorBinding<Payload>`. It constructs explicit bounded key claims and a retained `Fn(EditorKeyRequest) -> EditorDecision`. The factory executes while constructing the guest view. Only copied claims cross to the host; requests invoke the retained guest callback. No Rust closure or generated Message is serialized. Target::Native keeps the existing `fn(KeyPress, authored_args...) -> Option<iced::text_editor::Binding<Payload>>` contract. Tree previously refused this feature, so there is no prior Tree callback compatibility path to preserve.

`EditorBinding::new(claims, decide, on_event)` keeps the existing declared Payload route type. `on_event: Fn(EditorTransactionEvent) -> Option<Payload>` maps acknowledged native or guest commits, faults and cancellation; no decision carries a Message or user payload. The generated per-state transaction message first checks reset/observation and accepts the after-image, then runs this mapper and the authored route. Its Send message envelope retains only copied wire event and a context-local callback table id; closures stay guest-local. `Task::done` routes settle before the host admits the next queued input. Native commits report Native + kind + input time + before/after cursor; guest history alone chooses grouping. Patch history effects are NewGroup, ExtendPrevious, Undo and Redo.
