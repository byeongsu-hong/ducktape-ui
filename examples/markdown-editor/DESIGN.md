...
| --- | --- | --- |
| Unicode and IME | Every Hangul assembly stage appears in its own event cycle, uses the document font and baseline, participates in live wrapping, replaces a selection, and commits without moving the visual line | `hangul_ime_stages_relayout_before_the_next_key` drives `ㅇ → 으 → 응 → empty preedit → commit` as distinct IME events; `bundled_body_font_keeps_korean_ime_stages_on_one_baseline` and `preedit_uses_the_same_wrapped_paragraph_as_committed_text` verify the font and shared layout geometry |
