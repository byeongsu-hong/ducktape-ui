...
extern crate::adapters
  ...
  pure log_timeline_append(state:LogTimelineState) -> LogTimelineState
  pure log_timeline_resume(state:LogTimelineState) -> LogTimelineState
  component log_timeline(state:&LogTimelineState) -> LogTimelineEvent
  sync virtual_list_state() -> VirtualListState
  pure virtual_list_apply(state:VirtualListState, event:VirtualListEvent) -> VirtualListState
  ...
  pure tree_view_apply(state:TreeViewState, event:TreeViewEvent) -> TreeViewState
  pure tree_view_begin_selected_rename(state:TreeViewState) -> TreeViewState
  pure tree_view_cancel_rename(state:TreeViewState) -> TreeViewState
  ...
  pure data_grid_apply(state:DataGridState, event:DataGridEvent) -> DataGridState
  component data_grid(state:&DataGridState) -> DataGridEvent
  component aspect_ratio_demo() -> unit
  pure data_table_rows(query:str, sort:str, page:i64) -> [CatalogItem]
  pure data_table_page(query:str, page:i64) -> i64
  pure data_table_next_sort(sort:str) -> str
  pure data_table_can_next(query:str, page:i64) -> bool
  pure data_table_page_count(query:str) -> i64
  pure data_table_result_count(query:str) -> i64
