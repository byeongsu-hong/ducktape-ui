...
| 3c — declarative graphics and responsive layout | Canvas geometry data implemented; host-evaluated container rules remain, with widget-local opt-in measurements only. | Geometry rendering and interaction; multiple container widths with correct branches and no guest layout callback; bounded sensor feedback. |
