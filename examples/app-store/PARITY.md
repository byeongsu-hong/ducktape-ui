...
| First-class authored Ice tests | Native harness | Counter's same authored preset/state/dispatch/click/text and keyed-row scenarios run on native and Wasm test artifacts | Explicit test artifacts support presets, typed state/dispatch, literal-key targets, click, exists/missing and literal rendered text; input focus/typing/selection and keyboard actions reuse the mounted Driver on both backends (action arguments are literals); mounts and other actions remain follow-up |
