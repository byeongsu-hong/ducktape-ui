...
fn ticks_arrive_as_a_stream_and_move_the_display() {
    ...
    let frame = tick_native(vec![answer(now.id, &when)]);
    ...
    let frame = tick_native(vec![item(subscribe.id, &5_000_u64.to_le_bytes())]);
    ...
    let frame = tick_native(vec![item(subscribe.id, &305_000_u64.to_le_bytes())]);
