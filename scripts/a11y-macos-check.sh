#!/bin/sh
set -eu

# The macOS half of `scripts/a11y-windows-check.sh`: the NSAccessibility
# adapter only compiles for an Apple target, so this needs a Mac and no CI job
# runs it. It builds the runtime and the reference app in both their production
# and test forms, then runs the bridge tests that pin the adapter's
# main-thread refusal and its window-focus handling.

if [ "$(uname -s)" != "Darwin" ]; then
  echo "requires macOS; the NSAccessibility adapter has no cross-compile" >&2
  exit 1
fi

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
cd "$(dirname -- "$script_dir")"

cargo check --locked \
  -p ui-lang-runtime \
  -p showcase

cargo check --locked --tests \
  -p ui-lang-runtime \
  -p ui-lang-core \
  -p showcase

cargo test --locked -p ui-lang-runtime --lib -- macos_
