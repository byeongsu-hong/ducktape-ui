...
where
    ...
    P::Theme: theme::Base,
{
    run_inner(program, None, std::time::Duration::ZERO)
}

/// Runs a [`Program`] while interleaving work before each event-loop pump.
///
/// This is intended for native runtimes that must share the main thread with
/// `winit` and cannot safely perform their work from inside an event handler.
#[cfg(not(target_arch = "wasm32"))]
pub fn run_with_event_loop<P>(
    program: P,
    mut before_events: impl FnMut(),
...
where
    ...
    P::Theme: theme::Base,
{
    run_inner(program, Some(&mut before_events), pump_interval)
}

fn run_inner<P>(
    program: P,
    before_events: Option<&mut dyn FnMut()>,
    pump_interval: std::time::Duration,
) -> Result<(), Error>
where
    P: Program + 'static,
    P::Theme: theme::Base,
{
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        if let Some(before_events) = before_events {
            use winit::event_loop::ControlFlow;

            event_loop.set_control_flow(ControlFlow::Poll);
            loop {
                before_events();
                if let PumpStatus::Exit(_) = event_loop
                    .pump_app_events(Some(std::time::Duration::ZERO), &mut runner)
                {
                    break;
                }
