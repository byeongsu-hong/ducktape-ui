...
// A path dependency is built without `--cap-lints`, and the published source
// calls `try_next` on a channel whose newer release deprecates it.
#![allow(deprecated)]
#![doc(
                        ...
                        Ok(Some(control)) => match control {
                            Control::ChangeFlow(flow) => {
                                ...
                                match (event_loop.control_flow(), flow) {
                                    // Only a deadline still ahead is worth
                                    // keeping: one already reached was
                                    // consumed by the window that set it,
                                    // and left in place it wakes the loop
                                    // with a zero timeout until the later
                                    // one passes.
                                    (
