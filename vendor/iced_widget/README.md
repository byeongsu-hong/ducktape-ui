...
Only `src/row.rs` and `src/column.rs` change upstream behavior. Wrapping layouts
record their actual line ranges, resolve their assigned size before alignment,
and align each line against the inner content area. This fixes center/end
alignment in filling or fixed-size layouts and with leading padding. The
remaining source, generated manifest and assets match the published package.
See `examples/showcase/tests/cases/ui/wrap_alignment.ice` and the wrapping
alignment section in `COVERAGE.md` for executable Red/Green evidence.
