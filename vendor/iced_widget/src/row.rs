    ...
    Renderer: crate::core::Renderer,
{
    ...
    fn layout(
        ...
        // Ducktape patch: the wrapped lines, recorded as they are produced.
        // Horizontal alignment below used to rediscover them from the laid-out
        // coordinates, which cannot work once padding shifts every line start.
        ...
        lines.push(row_start..children.len());
        ...
        // Ducktape patch: resolve before aligning. `intrinsic_size` is the
        // width of the widest line, so aligning against it made every line
        // flush with that line and never with the space the row was actually
        // given — a `Fill` or larger `Fixed` row ended its lines short of its
        // own right edge. `resolve` is the only thing that knows the assigned
        // inner width, and it already collapses `Fill` back to the intrinsic
        // width when the parent compresses us, which keeps a shrink parent
        // aligning against content as before.
        let size =
            limits.resolve(self.row.width, self.row.height, intrinsic_size);

        if align_factor != 0.0 {
            // An unbounded `Fill` resolves to an infinite width, and an
            // infinite translation turns every child bound into NaN.
            let total_width = if size.width.is_finite() {
                size.width
            } else {
            ...
            for line in &lines {
                let Some(last) = children[line.clone()].last() else {
                    continue;
                };

                let bounds = last.bounds();
                // Children were placed at `padding.left`; line widths and
                // `total_width` are both measured from the content origin.
                let row_width =
                    bounds.x + bounds.width - self.row.padding.left;

                let translation =
                    Vector::new((total_width - row_width) / align_factor, 0.0);

                for node in &mut children[line.clone()] {
