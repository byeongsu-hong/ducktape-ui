    ...
    Renderer: text::Renderer,
{
    ...
    fn draw(
        ...
        for decoration in internal.editor.decorations() {
            let decoration_bounds = decoration.bounds + translation;
            let Some(decoration_bounds) =
                bounds.intersection(&decoration_bounds)
            else {
                continue;
            };
            ...
            renderer.fill_quad(
                renderer::Quad {
                    ...
                    border: decoration.highlight.border,
                    ..renderer::Quad::default()
                },
                decoration.highlight.background,
            );
        }
                            ...
                            .is_none_or(|preedit| preedit.content.is_empty()) =>
                {
                    let cursor =
                        Rectangle::new(
                            ...
                            Size::new(1.0, internal.editor.caret_height().into()),
                        );
    ...
    pub fn from_key_press(event: KeyPress) -> Option<Self> {
        ...
        match modified_key.as_ref() {
            ...
            keyboard::Key::Named(key::Named::Delete)
                if modifiers.jump()
                    && (text.is_none() || text.as_deref() == Some("\u{7f}")) =>
            {
                Some(Self::Sequence(vec![
                    Self::Select(Motion::WordRight),
                    Self::Delete,
                ]))
            }
            keyboard::Key::Named(key::Named::Delete)
