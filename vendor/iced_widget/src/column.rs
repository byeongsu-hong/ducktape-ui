    ...
    Renderer: crate::core::Renderer,
{
    ...
    fn layout(
        ...
        // Vertical alignment below used to rediscover them from the laid-out
        ...
        lines.push(column_start..children.len());
        ...
        // Ducktape patch: resolve before aligning. See `row.rs` for the full
        // argument; this is the same fix on the vertical axis.
        ...
        if align_factor != 0.0 {
            // An unbounded `Fill` resolves to an infinite height, and an
            // infinite translation turns every child bound into NaN.
            let total_height = if size.height.is_finite() {
                size.height
            } else {
            ...
            for line in &lines {
                ...
                // Children were placed at `padding.top`; line heights and
                // `total_height` are both measured from the content origin.
                let column_height =
                    bounds.y + bounds.height - self.column.padding.top;
