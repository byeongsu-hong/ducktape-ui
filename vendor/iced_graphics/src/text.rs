...
pub fn align(
    ...
    let (mut min_bounds, has_rtl) = measure(buffer);
    ...
    if let Some(align) = to_align(alignment) {
        ...
        if has_multiple_lines {
            // Justification distributes space across the available line, not
            // merely across the longest naturally shaped line. Unbounded
            // paragraphs still use their measured width.
            if alignment == Alignment::Justified {
                if let Some(width) =
                    buffer.size().0.filter(|width| width.is_finite())
                {
                    min_bounds.width = width.max(min_bounds.width);
                }
            }
    ...
    if needs_relayout {
        ...
        if alignment == Alignment::Justified {
            // Hard line breaks and unexpanded final lines retain their natural
            // width. Only actual justification can widen the measured result.
            // set_size skips layout when the dimensions already match, but
            // changing line alignment invalidates the cached layout anyway.
            buffer.shape_until_scroll(font_system, false);
            return measure(buffer).0;
        }
    }
...
mod alignment_tests {
    ...
    #[test]
    fn justified_width_follows_expanded_lines() {
        let mut fonts = cosmic_text::FontSystem::new_with_fonts([
            cosmic_text::fontdb::Source::Binary(Arc::new(
                include_bytes!("../fonts/FiraSans-Regular.ttf").as_slice(),
            )),
        ]);
        let mut shaped = |content: &str, width, height| {
            let mut buffer = cosmic_text::Buffer::new(
                &mut fonts,
                cosmic_text::Metrics::new(14.0, 20.0),
            );
            buffer.set_size(&mut fonts, width, Some(height));
            buffer.set_wrap(&mut fonts, cosmic_text::Wrap::Word);
            buffer.set_text(
                &mut fonts,
                content,
                &cosmic_text::Attrs::new()
                    .family(cosmic_text::Family::Name("Fira Sans")),
                cosmic_text::Shaping::Advanced,
                None,
            );
            let natural = measure(&buffer).0;
            let justified =
                align(&mut buffer, &mut fonts, Alignment::Justified);
            (natural, justified)
        };
        let (_, wrapped) = shaped(
            "Alpha beta gamma delta epsilon zeta end",
            Some(120.0),
            160.0,
        );
        assert_eq!(wrapped.width, 120.0);
        for (content, width, height) in [
            ("short", Some(120.0), 160.0),
            ("a\nb", Some(120.0), 160.0),
            ("a\nb", Some(120.0), 40.0),
            ("a\nb", None, 160.0),
        ] {
            let (natural, justified) = shaped(content, width, height);
            assert_eq!(
                justified, natural,
                "unexpanded text must keep natural bounds"
            );
        }
    }
}
