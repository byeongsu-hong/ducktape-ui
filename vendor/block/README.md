...
This workspace vendors `block` 0.1.6 because its upstream release models
`_NSConcreteStackBlock` with an uninhabited Rust type. The local copy uses
`c_void` for that opaque symbol and spells the existing C ABI explicitly so
current and future Rust compilers can build it without compatibility warnings.
