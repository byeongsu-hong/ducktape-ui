...
| Message Scroller | Shipped | Controlled stable-row transcript with initial start/end/last-anchor positioning, intent-aware live following, new-turn peek anchoring, restoration after prepends, resizes or deletion of a visible anchor, start/end/message alignment commands, derived visibility/current-anchor/edge/unread state, keyboard scrolling, and a jump-to-latest control. |
