...
mod tests {
    ...
    fn swipe_reducer_honors_direction_threshold_and_cancels_cleanly() {
        let mut state = SonnerState::default();
        state.set_offset(f32::INFINITY);
        state.set_swipe_threshold(f32::INFINITY);
        assert_eq!(state.offset(), DEFAULT_OFFSET);
        assert_eq!(state.swipe_threshold(), DEFAULT_SWIPE_THRESHOLD);
