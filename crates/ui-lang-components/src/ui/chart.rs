        ...
        CartesianKind::Bar(BarLayout::Stacked) => {
            for datum in points {
                ...
                let negative: f64 = datum
                    ...
                    .map(f64::from)
                    .sum();
                if positive > f64::from(f32::MAX) || negative < f64::from(f32::MIN) {
                    return Err(ChartError::InvalidDomain("y"));
                }
                let (positive, negative) = (positive as f32, negative as f32);
...
fn expanded_domain(min: f32, max: f32) -> AxisDomain {
    if min < max {
        ...
        let padding = (f64::from(min).abs() * 0.1).max(1.0);
        padded_domain(min, max, padding)
    }
}

fn bar_x_domain(points: &[PreparedDatum], min: f32, max: f32) -> AxisDomain {
    if min == max {
        return expanded_domain(min, max);
    }
    ...
    let edge = if gap.is_finite() {
        ...
        (f64::from(max) - f64::from(min)) * 0.5
    };
    padded_domain(min, max, edge)
}

fn padded_domain(min: f32, max: f32, padding: f64) -> AxisDomain {
    AxisDomain::new(
        (f64::from(min) - padding).max(f64::from(f32::MIN)) as f32,
        (f64::from(max) + padding).min(f64::from(f32::MAX)) as f32,
    )
}
...
fn map_x(value: f32, domain: AxisDomain, plot: Rectangle) -> f32 {
    plot.x
        + ((f64::from(value) - f64::from(domain.min))
            / (f64::from(domain.max) - f64::from(domain.min))) as f32
            * plot.width
}
...
pub fn pie_geometry(
    ...
    let scale = valid.iter().map(|(_, value)| *value).fold(0.0, f32::max);
    let total: f32 = valid.iter().map(|(_, value)| value / scale).sum();
    let mut angle = -PI / 2.0;
    let slices = valid
        .into_iter()
        .map(|(series_key, value)| {
            let start_angle = angle;
            angle += value / scale / total * TAU;
            PieSliceGeometry {
                series_key,
                value,
                start_angle,
...
mod tests {
    ...
    #[test]
    fn extreme_finite_values_never_create_nonfinite_geometry() {
        let extreme = ChartData::new([
            ChartDatum::new(-f32::MAX, "Low").with_value("desktop", -f32::MAX),
            ChartDatum::new(f32::MAX, "High").with_value("desktop", f32::MAX),
        ]);
        let geometry =
            cartesian_geometry(&config(), &extreme, bounds(), CartesianOptions::default()).unwrap();
        assert!(geometry.domain.x.min.is_finite() && geometry.domain.x.max.is_finite());
        assert!(geometry.domain.y.min.is_finite() && geometry.domain.y.max.is_finite());
        assert!(geometry.marks.iter().all(|mark| {
            match mark {
                CartesianMark::Line { points, .. } | CartesianMark::Area { points, .. } => points
                    .iter()
        ...
        let overflowing_stack = ChartData::new([ChartDatum::new(0.0, "Huge")
            .with_value("desktop", f32::MAX)
            .with_value("mobile", f32::MAX)]);
        assert_eq!(
            cartesian_geometry(
                &config(),
                &overflowing_stack,
            ...
            Err(ChartError::InvalidDomain("y"))
        );
    }

    #[test]
    fn geometry_rejects_nonfinite_bounds() {
        let invalid = Rectangle::new(Point::ORIGIN, Size::new(f32::INFINITY, 240.0));
        assert_eq!(
            cartesian_geometry(&config(), &data(), invalid, CartesianOptions::default()),
            Err(ChartError::TooSmall)
        );
        assert_eq!(
            pie_geometry(&config(), &PieData::new([("desktop", 1.0)]), invalid, 0.0,),
            Err(ChartError::TooSmall)
        );
    }
    ...
    fn donut_geometry_and_hit_testing_respect_the_hole() {
        ...
        let huge = pie_geometry(
            &config(),
            &PieData::new([("desktop", f32::MAX), ("mobile", f32::MAX)]),
            bounds(),
            0.0,
        )
        .unwrap();
        assert!(huge.slices.iter().all(|slice| {
            slice.start_angle.is_finite()
                && slice.end_angle.is_finite()
                && (slice.end_angle - slice.start_angle - PI).abs() < 0.001
        }));
    }
