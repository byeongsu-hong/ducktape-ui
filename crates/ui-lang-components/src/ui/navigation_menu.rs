...
impl HoverIntent {
    fn observe(&mut self, target: Option<usize>, now: Instant, delay: Duration) {
        ...
        self.pending = target.map(|target| (target, now.checked_add(delay).unwrap_or(now)));
    }
...
mod tests {
    ...
    #[test]
    fn overflowing_hover_delay_degrades_to_an_immediate_deadline() {
        let start = Instant::now();
        let mut hover = HoverIntent::default();

        hover.observe(Some(0), start, Duration::MAX);

        assert_eq!(hover.deadline(), Some(start));
    }
