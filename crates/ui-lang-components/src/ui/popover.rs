...
pub fn resolve_position(
    ...
    let anchor = sanitized_rectangle(anchor);
    let viewport = sanitized_rectangle(viewport);
    let content = Size::new(
        finite_nonnegative(content.width),
        finite_nonnegative(content.height),
    );
...
fn clamp_axis(position: f32, size: f32, viewport_start: f32, viewport_size: f32) -> f32 {
    let minimum = f64::from(viewport_start);
    let maximum = (minimum + f64::from(viewport_size) - f64::from(size)).max(minimum);
    ...
    position.clamp(minimum, maximum) as f32
}
...
fn sanitized_rectangle(bounds: Rectangle) -> Rectangle {
    let x = finite_or_zero(bounds.x);
    let y = finite_or_zero(bounds.y);
    Rectangle {
        x,
        y,
        width: representable_extent(x, bounds.width),
        height: representable_extent(y, bounds.height),
    }
}

fn representable_extent(origin: f32, extent: f32) -> f32 {
    f64::from(finite_nonnegative(extent))
        .min(f64::from(f32::MAX) - f64::from(origin))
        .max(0.0) as f32
}
...
mod tests {
    ...
    #[test]
    fn nonfinite_geometry_is_sanitized_before_positioning() {
        let resolved = resolve_position(
            rect(f32::NAN, f32::INFINITY, -20.0, f32::NAN),
            Size::new(f32::INFINITY, -10.0),
            rect(f32::NAN, f32::NEG_INFINITY, 320.0, 240.0),
            FloatingConfig::default(),
        );
        ...
        assert!(resolved.bounds.width >= 0.0 && resolved.bounds.height >= 0.0);

        let extreme = resolve_position(
            rect(f32::MAX, f32::MAX, f32::MAX, f32::MAX),
            Size::new(f32::MAX, f32::MAX),
            rect(f32::MAX, f32::MAX, f32::MAX, f32::MAX),
            FloatingConfig {
                side_offset: f32::MAX,
                alignment_offset: f32::MAX,
                ..FloatingConfig::default()
            },
        );
        assert!(
            [
                extreme.bounds.x,
                extreme.bounds.y,
                extreme.bounds.width,
                extreme.bounds.height,
            ]
            .into_iter()
            .all(f32::is_finite)
        );
