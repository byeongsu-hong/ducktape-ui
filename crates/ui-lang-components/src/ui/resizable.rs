...
fn normalize_minimums(panel_count: usize, minimums: &[f32]) -> Vec<f32> {
    let mut minimums = (0..panel_count)
        ...
        .collect::<Vec<_>>();
    let total = minimums.iter().copied().map(f64::from).sum::<f64>();

    if total > 1.0 {
        minimums
            .iter_mut()
            .for_each(|value| *value = (f64::from(*value) / total) as f32);
    }
...
fn normalize_sizes(panel_count: usize, sizes: &[f32], minimums: &[f32]) -> Vec<f32> {
    ...
    let mut desired = (0..panel_count)
        ...
        .collect::<Vec<_>>();
    let desired_total = desired.iter().copied().map(f64::from).sum::<f64>();

    if desired_total > 0.0 {
        desired
            .iter_mut()
            .for_each(|value| *value = (f64::from(*value) / desired_total) as f32);
    } else {
...
mod tests {
    ...
    fn layout_normalizes_bad_inputs_and_feasible_minimums() {
        ...
        let extreme = ResizableLayout::new(2, &[f32::MAX, f32::MAX / 2.0], &[f32::MAX, f32::MAX]);
        close(extreme.sizes().iter().sum(), 1.0);
        close(extreme.sizes()[0], 0.5);
        close(extreme.sizes()[1], 0.5);
        assert!(extreme.minimums().iter().all(|value| value.is_finite()));
    }
