...
fn fitted_size(bounds: Size, ratio: f32) -> Size {
    let width_from_height =
        || (f64::from(bounds.height) * f64::from(ratio)).min(f64::from(f32::MAX)) as f32;
    let width = match (bounds.width.is_finite(), bounds.height.is_finite()) {
        (true, true) => bounds.width.min(width_from_height()),
        (true, false) => bounds.width,
        (false, true) => width_from_height(),
        (false, false) => 0.0,
    }
    .max(0.0);

    let height = (f64::from(width) / f64::from(ratio)).min(f64::from(f32::MAX)) as f32;
...
mod tests {
    ...
    fn invalid_ratios_fall_back_to_a_square() {
        ...
        let extreme = fitted_size(Size::new(f32::INFINITY, f32::MAX), f32::MAX);
        assert!(extreme.width.is_finite() && extreme.height.is_finite());
        let extreme = fitted_size(Size::new(f32::MAX, f32::INFINITY), f32::MIN_POSITIVE);
        assert!(extreme.width.is_finite() && extreme.height.is_finite());
    }
}
