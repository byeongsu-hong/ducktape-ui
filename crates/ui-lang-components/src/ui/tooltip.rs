...
impl DelayedPresence {
    ...
    pub(crate) fn set_active(
        ...
        self.phase = match (active, self.phase) {
            ...
            (true, Phase::Closed) => Phase::Opening(now.checked_add(open_delay).unwrap_or(now)),
            (true, Phase::Closing(_)) => Phase::Open,
            (true, phase) => phase,
            (false, Phase::Opening(_)) => Phase::Closed,
            (false, Phase::Open) if close_delay.is_zero() => Phase::Closed,
            (false, Phase::Open) => Phase::Closing(now.checked_add(close_delay).unwrap_or(now)),
            (false, phase) => phase,
        };
    }
...
mod tests {
    ...
    #[test]
    fn overflowing_delays_degrade_to_an_immediate_deadline() {
        let start = Instant::now();
        let mut presence = DelayedPresence::default();

        presence.set_active(true, start, Duration::MAX, Duration::ZERO);

        assert_eq!(presence.deadline(), Some(start));
    }
