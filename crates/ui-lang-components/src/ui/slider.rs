...
impl SliderSpec {
    pub fn new(range: RangeInclusive<f32>, step: f32) -> Self {
        ...
        Self {
            ...
            max,
            step,
            page_step: (f64::from(step) * 10.0).min(f64::from(f32::MAX)) as f32,
        }
    }
...
fn snap(value: f32, spec: SliderSpec) -> f32 {
    ...
    let min = f64::from(spec.min);
    let max = f64::from(spec.max);
    let steps = ((f64::from(value) - min) / f64::from(spec.step)).round();
    (min + steps * f64::from(spec.step)).clamp(min, max) as f32
}
...
fn normalized(value: f32, spec: SliderSpec) -> f32 {
    if spec.min == spec.max {
        ...
        ((f64::from(value) - f64::from(spec.min)) / (f64::from(spec.max) - f64::from(spec.min)))
...
fn value_from_point(
    ...
    snap(
        (f64::from(spec.min) + f64::from(logical) * (f64::from(spec.max) - f64::from(spec.min)))
            as f32,
        spec,
    )
}
...
mod tests {
    ...
    fn invalid_numeric_inputs_have_boring_safe_defaults() {
        ...
        let extreme = SliderSpec::new(-f32::MAX..=f32::MAX, f32::MAX);
        assert!(extreme.page_step.is_finite());
        assert_eq!(normalized(-f32::MAX, extreme), 0.0);
        assert_eq!(normalized(0.0, extreme), 0.5);
        assert_eq!(normalized(f32::MAX, extreme), 1.0);
        assert!(snap(0.0, extreme).is_finite());
    }
