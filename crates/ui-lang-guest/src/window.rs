...
pub fn perform<M: MaybeSend + 'static>(command: wire::WindowCommand) -> iced::Task<M> {
...
mod tests {
    ...
    impl App for WindowApp {
        ...
        fn boot() -> (Self, iced::Task<bool>) {
            (
                Self(false),
                perform(wire::WindowCommand::Focus).chain(iced::Task::done(true)),
            )
        }
        fn view(&self) -> wire::Node {
            wire::Node::empty()
        }
        fn update(&mut self, submitted: bool) -> iced::Task<bool> {
            self.0 = submitted;
            iced::Task::none()
        }
