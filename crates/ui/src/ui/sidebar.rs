...
/// Default Ducktape sidebar measurements, in logical pixels.
...
impl SidebarMetrics {
    pub fn menu_height(self, size: SidebarMenuButtonSize) -> f32 {
        ...
        match size {
            SidebarMenuButtonSize::Small => metrics.menu_small_height,
            SidebarMenuButtonSize::Default => metrics.menu_default_height,
            SidebarMenuButtonSize::Large => metrics.menu_large_height,
        }
    }
        ...
        match viewport {
            SidebarViewport::Mobile => {
                if state.mobile_open {
                    metrics.mobile_width
                } else {
                    0.0
                }
            }
            SidebarViewport::Desktop => match collapsible {
                SidebarCollapsible::None => metrics.desktop_width,
                SidebarCollapsible::Offcanvas if !state.expanded => 0.0,
                SidebarCollapsible::Icon if !state.expanded => metrics.icon_width,
                SidebarCollapsible::Offcanvas | SidebarCollapsible::Icon => metrics.desktop_width,
            },
        }
    }

    fn sanitized(self) -> Self {
        let value = |value: f32, fallback: f32| {
        ...
        Self {
            desktop_width: value(self.desktop_width, SIDEBAR_METRICS.desktop_width),
            mobile_width: value(self.mobile_width, SIDEBAR_METRICS.mobile_width),
            icon_width: value(self.icon_width, SIDEBAR_METRICS.icon_width),
            floating_padding: value(self.floating_padding, SIDEBAR_METRICS.floating_padding),
            section_padding: value(self.section_padding, SIDEBAR_METRICS.section_padding),
            group_label_height: value(self.group_label_height, SIDEBAR_METRICS.group_label_height),
            menu_gap: value(self.menu_gap, SIDEBAR_METRICS.menu_gap),
            menu_small_height: value(self.menu_small_height, SIDEBAR_METRICS.menu_small_height),
            menu_default_height: value(
                self.menu_default_height,
                SIDEBAR_METRICS.menu_default_height,
            ),
            menu_large_height: value(self.menu_large_height, SIDEBAR_METRICS.menu_large_height),
            compact_control_size: value(
                self.compact_control_size,
                SIDEBAR_METRICS.compact_control_size,
            ),
            rail_hit_size: value(self.rail_hit_size, SIDEBAR_METRICS.rail_hit_size),
            seam_width: value(self.seam_width, SIDEBAR_METRICS.seam_width),
            submenu_height: value(self.submenu_height, SIDEBAR_METRICS.submenu_height),
        }
    }
}
    ...
    Message: Clone + 'a,
{
    ...
    pub fn metrics(mut self, metrics: SidebarMetrics) -> Self {
        self.metrics = metrics.sanitized();
        self
    }
...
mod tests {
    ...
    #[test]
    fn invalid_custom_metrics_fall_back_without_poisoning_layout() {
        let metrics = SidebarMetrics {
            desktop_width: f32::NAN,
            mobile_width: f32::INFINITY,
            menu_default_height: -1.0,
            ..SIDEBAR_METRICS
        }
        .sanitized();

        assert_eq!(metrics.desktop_width, SIDEBAR_METRICS.desktop_width);
        assert_eq!(metrics.mobile_width, SIDEBAR_METRICS.mobile_width);
        assert_eq!(
            metrics.menu_height(SidebarMenuButtonSize::Default),
            SIDEBAR_METRICS.menu_default_height
        );
    }
