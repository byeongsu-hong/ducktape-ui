...
pub fn resolve_sheet_geometry(
    ...
    let x = finite_or_zero(viewport.x);
    let y = finite_or_zero(viewport.y);
    let viewport = Rectangle {
        x,
        y,
        width: representable_extent(x, viewport.width),
        height: representable_extent(y, viewport.height),
    };
...
mod tests {
    ...
    fn dimensions_are_capped_to_requested_max_fraction_and_viewport() {
        ...
        let invalid_viewport = resolve_sheet_geometry(
            Rectangle::new(
        ...
        assert!(
            [
                invalid_viewport.panel.x,
                invalid_viewport.panel.y,
                invalid_viewport.panel.width,
                invalid_viewport.panel.height,
            ]
            .into_iter()
            .all(f32::is_finite)
        );

        let extreme_viewport = resolve_sheet_geometry(
            Rectangle::new(
        ...
        assert!(
            [
                extreme_viewport.panel.x,
                extreme_viewport.panel.y,
                extreme_viewport.panel.width,
                extreme_viewport.panel.height,
            ]
            .into_iter()
            .all(f32::is_finite)
        );
