...
impl MessageScrollerViewport {
    fn sanitized(self) -> Self {
        let viewport_height = finite_nonnegative(self.viewport_height, 0.0);
        let content_height = finite_nonnegative(self.content_height, 0.0);
        let max_offset = (content_height - viewport_height).max(0.0);

        Self {
            offset_from_end: finite_nonnegative(self.offset_from_end, 0.0).min(max_offset),
            viewport_height,
            content_height,
        }
    }
...
mod tests {
    ...
    #[test]
    fn public_viewport_events_cannot_poison_scroll_state() {
        let mut state = MessageScrollerState::new("invalid-viewport");
        let items: Arc<[MessageScrollerItemMeta]> = Arc::from([]);

        let _ = state.update(MessageScrollerEvent::ViewportChanged {
            viewport: MessageScrollerViewport {
                offset_from_end: f32::INFINITY,
