...
pub fn reduce_menu(
    ...
    match command {
        ...
        MenuCommand::Activate | MenuCommand::Forward => {
            ...
            let Some(item) = items
                .iter()
                .find_map(|(visible, item)| (visible == &path && !item.disabled).then_some(*item))
            else {
                return MenuTransition {
                    state: next,
                    event: None,
...
mod tests {
    ...
    #[test]
    fn stale_hidden_focus_cannot_activate_an_unrendered_item() {
        let entries = entries();
        let stale = MenuState {
            focused: Some(vec![2, 0]),
            ..MenuState::default()
        };

        let transition = reduce_menu(&stale, &entries, MenuCommand::Activate);
