...
fn run() -> Result<(), String> {
    ...
    match command {
        ...
        "help" | "--help" | "-h" => {
            println!(
                "cargo ice <fmt [--check] | check | test [cargo-test args...] | clippy | compat | expand <file.ice> | dev <-p package | file.ice [-- cargo-build-args...]> [-- app-args...] | bundle -p <package>... [--target <triple>]... [--manifest-path <Cargo.toml>] [--out <dir>] [--no-wasm-opt] | inspect <file.ice> [--trace|--fuzz interactions|--replay trace.json] | diff <baseline.json> <current.json> [options] | api <root.ice> | api diff <baseline.json> <current.json> [--format human|json] | review <file.ice> [--bin <name>] [--trace] | schema | lsp>"
            );
            return Ok(());
        }
        _ => {}
...
mod tests {
    ...
    fn performance_contract_orphan_files_stream_without_scratch() {
        ...
        assert_eq!(measured, (0, 0), "orphan scan allocations: {measured:?}");
        eprintln!(
