...
fn expr_builtin_group_6(
    ...
    Ok(Some(match name {
        ...
        // Tree editors hold copied text/cursor state; native editors hold Content.
        "editor" if context.program.target() == Target::Tree => format!(
            "::ui_lang_guest::Editor::new({})",
            expr_node_code(args.value(0)?, env, context, ValueMode::Owned)?
        ),
        "editor" => format!(
        ...
        "editor_copy" if context.program.target() == Target::Tree => format!(
            "({}).clone()",
            expr_node_code(args.value(0)?, env, context, ValueMode::Borrowed)?
        ),
        "editor_text" => format!(
        ...
        "editor_line" if context.program.target() == Target::Tree => format!(
            "::std::convert::TryFrom::try_from({}).ok().and_then(|__line: usize| ({}).line(__line))",
            expr_node_code(args.value(1)?, env, context, ValueMode::Owned)?,
            expr_node_code(args.value(0)?, env, context, ValueMode::Borrowed)?
        ),
        "editor_line" => format!(
...
fn resolved_expr_use_code_in(
    ...
    Ok(match &expression_use.coercion {
        ...
        ResolvedInitializerCoercion::StrToEditor if context.program.target() == Target::Tree => {
            format!("::ui_lang_guest::Editor::new({code})")
        }
