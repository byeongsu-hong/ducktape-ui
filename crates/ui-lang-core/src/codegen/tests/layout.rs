...
#[test]
fn only_a_direct_pin_layer_of_a_plain_stack_floats() {
    // A `pin` layer of a `stack` is positioned, so it must not vote on the
    // stack's size — a popover pinned under a header row must not grow the
    // header. Nothing else acquires the sink: an ordinary stack keeps its
    // byte-identical output, a pin nested in a `col` is a layer of the COL, and
    // `stack under=N` builds iced's own base-sized `Stack`.
    let app = |view: &str| {
        format!(
            r#"app Popover
theme contract AppTheme
...
{view}"#
    ...
    let base = "    box w=1.0 h=1.0 @bg-bg\n      text \"\"\n";
    let menu = "      box w=190.0 p=4.0 @bg-bg\n        text \"Menu\"\n";

    let plain = compile(
        &app(&format!(
            "  stack\n{base}    box w=190.0 p=4.0 @bg-bg\n      text \"Menu\"\n"
        )),
        "popover.ice",
    )
    .unwrap();
    assert!(!plain.contains("__ice_floating"));

    let direct = compile(
        &app(&format!("  stack\n{base}    pin x=0.0 y=38.0\n{menu}")),
        "popover.ice",
    )
    .unwrap();
    assert!(direct.contains("__ice_floating.push(__children.len() - 1);"));
    assert!(direct.contains("::ui_lang_runtime::zstack(__children).floating(__ice_floating)"));

    // One level deeper: the pin is a layer of the `col`, which does not float.
    let nested = compile(
        &app(&format!(
            "  stack\n{base}    col\n      pin x=0.0 y=38.0\n        box w=190.0 p=4.0 @bg-bg\n          text \"Menu\"\n"
        )),
    ...
    .unwrap();
    assert!(nested.contains("::iced::widget::pin("));
    assert!(!nested.contains("__ice_floating"));

    // `under=N` lowers to iced's native `Stack`, which already sizes to its base.
    let under = compile(
        &app(&format!(
            "  stack under=1\n{base}    pin x=0.0 y=38.0\n{menu}"
        )),
    ...
    .unwrap();
    assert!(under.contains("::iced::widget::pin("));
    assert!(under.contains("::iced::widget::Stack::new()"));
    assert!(!under.contains("__ice_floating"));
}
