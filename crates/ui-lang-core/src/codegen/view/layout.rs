...
fn render_resolved_regular_layout(
    ...
    write!(
        body,
        " let __a11y_key = {accessibility_key}; ::ui_lang_runtime::accessible(__layout_content, ::ui_lang_runtime::StableId::new(&__a11y_key), ::ui_lang_runtime::Role::GenericContainer).logical_id_maybe(::core::cfg!(test).then_some(__a11y_key)).into() }}"
    )
    .unwrap();
    Ok(body)
}
...
fn render_resolved_flexbox(
    ...
    write!(
        body,
        "; let __layout_content: __IceElement<'_, {message}> = __content.into(); let __a11y_key = {accessibility_key}; ::ui_lang_runtime::accessible(__layout_content, ::ui_lang_runtime::StableId::new(&__a11y_key), ::ui_lang_runtime::Role::GenericContainer).logical_id_maybe(::core::cfg!(test).then_some(__a11y_key)).into() }}"
    )
    .unwrap();
    Ok(body)
}
...
fn render_resolved_scroll(
    ...
    Ok(format!(
        "{{ let __a11y_key = {accessibility_key}; let __scroll_content: __IceElement<'_, {message}> = {child}; let __layout = {code}; ::ui_lang_runtime::accessible(__layout, ::ui_lang_runtime::StableId::new(&__a11y_key), ::ui_lang_runtime::Role::GenericContainer).logical_id_maybe(::core::cfg!(test).then_some(__a11y_key)).into() }}"
    ))
}
