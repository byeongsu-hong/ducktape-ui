...
| --- | --- |
| `text` | AccessKit `Label` with the visible text as its value; `live=polite\|assertive` sets the node's live region, which every native adapter turns into a screen-reader announcement when the value changes |
