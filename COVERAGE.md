...
| `ScrollIntoView` | every node inside an identified `scroll` (`scroll #id`, or a `view` tree, which identifies its own) advertises the action; the request walks the tree when it arrives, finds the nearest identified scroll around the node, and moves it only as far as the node's leading or trailing edge needs, so nothing is stored per node. Ice tests drive it with `a11y scroll-into-view target`; the step is an error for a node no identified scroll encloses. Not covered: an unidentified `scroll` in between, whose offset the request cannot see, and the `expect a11y … action` form, which stays click/focus. Evidence: a runtime test puts a button 500px down a 100px scroll, dispatches `ScrollIntoView`, and reads the node back inside the viewport; the showcase test `accessibility_scroll_into_view_reaches_an_offscreen_control` scrolls the catalog to `open-dialog`. Red: with the reveal arithmetic forced to "already visible", both fail (`still offscreen: y0: 500.0`; `expect open_dialog.visible … actual: false`) |
