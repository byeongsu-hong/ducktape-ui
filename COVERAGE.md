...
| component use | native | a layout memo the compiler inserts at every use whose arguments, slot content, body, and nested bodies read only app state, derived values, the instance's own state, palette entries, and subtree-declared locals, over layout-pure widgets: the layout node is keyed on those revisions and the incoming limits, the element is still built on every pass (the `memo_*` codegen tests pin the eligible and the refused shapes, the `rev_memo` runtime tests pin hit, miss, limits slots, and site reset, and every example suite passes byte-identical captures under it); a row-local or `match`-payload argument, a secret, an implicit animation clock read, a nested stateful component, an extern component, an editor, a `virtual-row` column, an auto-scrolling scrollable, a sensor, a resize handle, a responsive size, a float, a pane grid, or media leaves the use unmemoized |
