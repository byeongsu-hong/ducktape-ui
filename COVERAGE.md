...
and reports parsed-file, rechecked/reused-root, resolved-symbol, codegen-root,
and phase-timing counters. Focused fixtures prove two independent roots reuse
one checked result after an unrelated edit, a shared fragment invalidates every
dependent root, an overlay close returns to disk, byte-identical content is
reused, and transitive reverse edges are retained. LSP diagnostics, the dev
