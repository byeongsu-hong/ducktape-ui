...
| system tray | native | an Ice-owned surface beyond iced (backed by the `tray-icon` crate on macOS): the `tray` block's required codec-free RGBA icon with the same checked byte-length contract as window icons, macOS template flag, reactive `label`/`tooltip` `str` expressions re-evaluated per update and applied natively only on change, daemon-gated `popover` left-click toggle (hidden open, physical-to-logical anchoring through the popover's scale factor, monitor clamp, close-event tracking), restore-and-focus default click without a popover, and compiled no-op stubs on every other target; evidence: `tray-basic` compile, four tray diagnostics, and `tray-block` format fixtures, runtime anchor unit tests, and the trading `menubar` daemon's generated-toggle unit test plus rendered popover captures |
